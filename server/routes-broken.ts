...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // مسار فارغ للبثوث (تم حذف نظام البث)
  ...
  // مسارات البث محذوفة (تم إزالة نظام البث)
  // //app.post('/api/streams', requireAuth, async (req: any, res) => { (removed)
    try {
  ...
  //app.delete('/api/streams/:id', requireAuth, async (req: any, res) => { (removed)
    try {
  ...
  //app.get('/api/streams/:id', async (req, res) => { (removed)
    try {
      const streamId = parseInt(req.params.id);
  ...
  //app.post('/api/streams/:id/end', requireAuth, async (req: any, res) => { (removed)
    try {
      const streamId = parseInt(req.params.id);
  ...
  //app.get('/api/streams/:id/comments', async (req, res) => { (removed)
    try {
      const postId = parseInt(req.params.id);
      res.json([]);
    } catch (error) {
  ...
  //app.post('/api/streams/:id/comments', requireAuth, async (req: any, res) => { (removed)
    try {
      const postId = parseInt(req.params.id);
      const { content } = req.body;
  ...
  // Chat routes
  //app.get('/api/streams/:id/chat', async (req, res) => { (removed)
    try {
      const streamId = parseInt(req.params.id);
